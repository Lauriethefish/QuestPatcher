...
{
    /// <summary>
    /// The main class that manages most Quatcher services.
    /// Allows user prompts etc. to be abstracted through 
    /// </summary>
    public abstract class QuatcherService : INotifyPropertyChanged
    {
        ...
        protected QuatcherService(IUserPrompter prompter)
        {
            Prompter = prompter;
            SpecialFolders = new SpecialFolders(); // Load Quatcher application folders
            ...
            Logger.Debug($"QuatcherService constructed (Quatcher version {VersionUtil.QuatcherVersion})");
        }
        ...
        {
            HasLoaded = false;
            Logger.Information("Starting Quatcher . . .");
