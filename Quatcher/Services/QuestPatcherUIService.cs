...
namespace Quatcher.Services
{
    /// <summary>
    /// Implementation of QuatcherService that uses UI message boxes and creates the viewmodel for displaying in UI
    /// </summary>
    public class QuatcherUIService : QuatcherService
    {
        ...
        public QuatcherUIService(IClassicDesktopStyleApplicationLifetime appLifetime) : base(new UIPrompter())
        {
        ...
        private async Task LoadAndHandleErrors()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Logger.Error($"Exiting Quatcher due to unhandled load error: {ex}");
                    ExitApplication();
                }
            }   finally
            {
        ...
        private async void OnMainWindowClosing(object? sender, CancelEventArgs args)
        {
            ...
            DialogBuilder builder = new()
            {
                ...
                Text = "Quatcher still has ongoing operations. Closing Quatcher before these finish may lead to corruption of your install!"
            };
